...
function ReShadeInstallerSection() {
  ...
  const [addonEnabled, setAddonEnabled] = useState<boolean>(true);
  const [autoHdrEnabled, setAutoHdrEnabled] = useState<boolean>(true);
  ...
  useEffect(() => {
    const loadAutoHdrPref = async () => {
      try {
        ...
        if (result.status === "success") {
          setAutoHdrEnabled(result.autohdr_enabled ?? true);
        }
      } catch (e) {
    ...
    <PanelSection title="HDR Runtime">
      {pathExists !== null && (
        <PanelSectionRow>
          <div style={{ color: pathExists ? "green" : "red" }}>
            {pathExists ? (
              <>
                HDR ReShade runtime is installed
                {currentVersionInfo && (
              ...
              "HDR ReShade runtime is not installed"
            )}
          </div>
        </PanelSectionRow>
      )}
      ...
      {pathExists === false && (
        <PanelSectionRow>
          <div style={{ fontSize: '0.9em', opacity: 0.8 }}>
            Installs ReShade with addon support and AutoHDR components for Steam Deck OLED. Advanced shader package selection is still available after the runtime is installed.
          </div>
        </PanelSectionRow>
      )}
      ...
      <PanelSectionRow>
        <ToggleField
          ...
          description={pathExists ? "Required for AutoHDR/RenoDX; changes require reinstallation" : "Required for AutoHDR/RenoDX"}
      ...
      {addonEnabled && (
        <PanelSectionRow>
          <ToggleField
            ...
            description="Preset for Steam Deck OLED HDR fallback (DX10/11/12 only)"
      ...
      {pathExists === true && configChanged && (
        <PanelSectionRow>
          <div style={{
            ...
            Configuration changed - reinstall to apply changes
          </div>
        </PanelSectionRow>
      )}
        ...
        <div>
          ...
          {addonEnabled && autoHdrEnabled && (
            <div style={{ fontSize: '0.9em', marginTop: '4px', opacity: 0.8 }}>
              AutoHDR works with DirectX 10/11/12 games only. Use RenoDX import for game-specific profile addons.
            </div>
          )}
        </div>
      </PanelSectionRow>
